...
def now_utc() -> datetime:
    """Returns current local time (Czech time)"""
    return datetime.now()
...
def fetch_nhl_games(season: str = "20252026", team: str = None) -> List[Dict]:
    ...
    try:
        # NHL API endpoint
        url = f"https://api-web.nhle.com/v1/schedule/{season}"
        if team:
        ...
        for week in game_weeks:
            for game in week.get('games', []):
                ...
                # Skip preseason/playoff pokud nechceme
                if game_type != 2:
                    continue
                ...
                start_time = datetime.fromisoformat(start_time_utc.replace('Z', '+00:00')) if start_time_utc else None
        ...
        print(f"❌ Chyba při stahování NHL dat: {e}")
        return []
...
def fetch_football_games(league_id: int, api_key: str, season: int = None) -> List[Dict]:
    ...
    try:
        ...
        print(f"❌ Chyba při stahování fotbalových dat: {e}")
        return []


def fetch_api_games(api_source: APISource) -> List[Dict]:
    """
    Univerzální funkce pro stažení zápasů podle typu API
    
    Args:
        api_source: Instance APISource modelu
    
    Returns:
        List zápasů
    """
    api_type = api_source.api_type.lower()
    
    if api_type == 'nhl':
        season = api_source.league_id or "20252026"
        return fetch_nhl_games(season=season)
    
    elif api_type == 'api-football':
  ...
  <div class="api-source-card {% if source.is_active %}active{% endif %}">
    <div class="row" style="justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
      <div>
        ...
        <h3 style="margin: 0 0 4px 0;">{{ source.round.name }}</h3>
        <div class="muted" style="font-size: 13px;">
      ...
      <div class="row" style="gap: 8px;">
        ...
        <form method="post" action="{{ url_for('admin_api_source_delete', source_id=source.id) }}" 
              ...
              onsubmit="return confirm('Opravdu smazat API zdroj \'{{ source.round.name }}\'?')">
          <input type="hidden" name="csrf_token" value="{{ csrf_token() }}"/>
          <button type="submit" class="btn btn-sm btn-danger" 
                  style="background:rgba(255,77,109,0.2); color:#ff4d6d; border:none; cursor:pointer;">
            🗑️ Smazat
    ...
    <div style="display: grid; gap: 16px;">
      ...
      <div>
        ...
        <select name="api_type" id="api_type" required onchange="toggleAPIFields()">
          ...
          <option value="api-football">⚽ API-Football (fotbal)</option>
        </select>
      </div>
      
      <div id="nhl_fields" style="display: none;">
        <label class="muted" style="margin-bottom: 6px; display: block;">Sezóna</label>
        <input name="league_id" placeholder="20252026" value="20252026">
        <div class="muted" style="font-size: 12px; margin-top: 4px;">
      ...
      <div id="football_fields" style="display: none;">
        <div style="margin-bottom: 12px;">
          ...
          <input name="league_id" placeholder="39">
          <div class="muted" style="font-size: 12px; margin-top: 4px;">
            Např: 39 = Premier League, 140 = La Liga, 78 = Bundesliga<br>
        ...
        <div>
          ...
          <input name="api_key" type="password" placeholder="tvůj-api-klíč">
          <div class="muted" style="font-size: 12px; margin-top: 4px;">
...
function toggleAPIFields() {
  ...
  document.getElementById('nhl_fields').style.display = apiType === 'nhl' ? 'block' : 'none';
  document.getElementById('football_fields').style.display = apiType === 'api-football' ? 'block' : 'none';
}
</script>
""", rounds=rounds)
    ...
    def admin_api_source_edit(source_id: int):
        ...
        if request.method == "POST":
            source.round_id = int(request.form["round_id"])
            source.api_type = request.form["api_type"]
            source.league_id = request.form.get("league_id", "").strip() or None
            source.api_key = request.form.get("api_key", "").strip() or None
            
            source.auto_import_matches = request.form.get("auto_import_matches") == "on"
            source.auto_import_results = request.form.get("auto_import_results") == "on"
            source.require_admin_approval = request.form.get("require_admin_approval") == "on"
            source.exclude_overtime = request.form.get("exclude_overtime") == "on"
            source.is_active = request.form.get("is_active") == "on"
            
            db.session.commit()
            
            audit("api_source.edit", "APISource", source.id)
...
<div class="card">
  ...
  <div class="muted">{{ source.round.name }} - {{ source.api_type.upper() }}</div>
  <hr class="sep">
  ...
  <form method="post">
    ...
    <div style="display: grid; gap: 16px;">
      ...
      <div>
        ...
        <select name="round_id" required>
          {% for r in rounds %}
            <option value="{{ r.id }}" {% if r.id == source.round_id %}selected{% endif %}>{{ r.name }}</option>
          {% endfor %}
        </select>
      </div>
      ...
      <div>
        <label class="muted" style="margin-bottom: 6px; display: block;">Typ API *</label>
        <select name="api_type" id="api_type" required onchange="toggleAPIFields()">
          <option value="nhl" {% if source.api_type == 'nhl' %}selected{% endif %}>🏒 NHL API (hokej)</option>
          <option value="api-football" {% if source.api_type == 'api-football' %}selected{% endif %}>⚽ API-Football (fotbal)</option>
        </select>
      </div>
      
      <div id="nhl_fields" style="{% if source.api_type != 'nhl' %}display: none;{% endif %}">
        <label class="muted" style="margin-bottom: 6px; display: block;">Sezóna</label>
        <input name="league_id" placeholder="20252026" value="{{ source.league_id or '20252026' }}">
        <div class="muted" style="font-size: 12px; margin-top: 4px;">
          Formát: YYYYYYYY (např. 20252026 pro sezónu 2025/26)
        </div>
      </div>
      
      <div id="football_fields" style="{% if source.api_type != 'api-football' %}display: none;{% endif %}">
        <div style="margin-bottom: 12px;">
          <label class="muted" style="margin-bottom: 6px; display: block;">ID Ligy *</label>
          <input name="league_id" placeholder="39" value="{{ source.league_id or '' }}">
          <div class="muted" style="font-size: 12px; margin-top: 4px;">
            Např: 39 = Premier League, 140 = La Liga, 78 = Bundesliga<br>
            <a href="https://www.api-football.com/documentation-v3#tag/Leagues" target="_blank">Najdi ID ligy →</a>
          </div>
        ...
        <div>
          <label class="muted" style="margin-bottom: 6px; display: block;">API Klíč *</label>
          <input name="api_key" type="password" placeholder="{% if source.api_key %}••••••••{% else %}tvůj-api-klíč{% endif %}" value="{{ source.api_key or '' }}">
          <div class="muted" style="font-size: 12px; margin-top: 4px;">
            Registrace: <a href="https://www.api-football.com/" target="_blank">api-football.com</a>
          </div>
        </div>
      ...
      <div>
        ...
        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 8px;">
          <input type="checkbox" name="auto_import_matches" {% if source.auto_import_matches %}checked{% endif %}>
          <span>Automatický import zápasů</span>
        </label>
        
        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 8px;">
          <input type="checkbox" name="auto_import_results" {% if source.auto_import_results %}checked{% endif %}>
          <span>Automatický import výsledků</span>
        </label>
        
        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 8px;">
          <input type="checkbox" name="require_admin_approval" {% if source.require_admin_approval %}checked{% endif %}>
          <span>Vyžadovat potvrzení adminem</span>
        </label>
        
        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; margin-bottom: 8px;">
          <input type="checkbox" name="exclude_overtime" {% if source.exclude_overtime %}checked{% endif %}>
...
function toggleAPIFields() {
  ...
  document.getElementById('nhl_fields').style.display = type === 'nhl' ? 'block' : 'none';
  document.getElementById('football_fields').style.display = type === 'api-football' ? 'block' : 'none';
}
</script>
""", source=source, rounds=rounds)
    ...
    def admin_api_source_delete(source_id: int):
        ...
        source_name = f"{source.round.name} - {source.api_type.upper()}"
        
        # Smazat všechny import logy tohoto zdroje
        APIImportLog.query.filter_by(source_id=source.id).delete()
        
        # Smazat všechny mapování
        MatchAPIMapping.query.filter_by(source_id=source.id).delete()
    ...
    def admin_api_import_preview(source_id: int, import_type: str):
        ...
        # Stáhnout data z API
        try:
            games = fetch_api_games(source)
            
            if not games:
                flash("❌ Nepodařilo se stáhnout data z API", "error")
                return redirect(url_for("admin_api_sources"))
        ...
        if request.method == "POST":
            ...
            if action == "approve":
                ...
                try:
                    ...
                    # Stáhnout data znovu (pro jistotu)
                    games = fetch_api_games(source)
                    
                    if import_log.import_type == 'matches':
                        imported, skipped, errors = import_matches_from_api(source, games, commit=True)
                    else:  # results
                        imported, skipped, errors = import_results_from_api(source, games, commit=True)
                    ...
                    flash(f"✅ Import dokončen! Importováno: {imported}, Přeskočeno: {skipped}", "ok")
