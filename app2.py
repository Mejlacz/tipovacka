...
""", u=u)
    ...
    def _parse_csv_style(line: str) -> Optional[Dict]:
        ...
        def _apply_dt_from_tail(tokens: list[str]) -> None:
            ...
            if not tokens:
                return
            last = tokens[-1]
            if leading_dt and re.match(r'^\d{1,2}:\d{2}$', last):
    ...
    def _parse_pipe_style(line: str) -> Optional[Dict]:
        ...
        if leading_date and tail and re.match(r'^\d{1,2}:\d{2}$', tail[0]):
        ...
        <div class="card">
          ...
          <form method="POST">
            ...
            <div class="form-group">
              ...
              <div style="margin:10px 0; padding:10px; border:1px solid #e5e7eb; border-radius:12px; background:#f8fafc;">
                <div style="font-weight:700; margin-bottom:6px;">Režim importu</div>
                <label style="display:block; margin:6px 0;">
                  <input type="radio" name="import_mode" value="fixtures" {% if import_mode != 'results' %}checked{% endif %}>
                  <strong>Zápasy</strong> (neodehrané) – skóre se ignoruje
                </label>
                <label style="display:block; margin:6px 0;">
                  <input type="radio" name="import_mode" value="results" {% if import_mode == 'results' %}checked{% endif %}>
                  <strong>Výsledky</strong> – aktualizovat existující zápasy
                </label>
              </div>
