...
""", u=u)
    ...
    def _parse_single_line(line: str, line_num: int = 0) -> Optional[Dict]:
        """
        Parsuje jeden řádek - zkouší různé formáty
        """
        ...
        parsers = [
            ...
            _parse_datetime_first,     # 14.2. 20:00 Sparta vs Slavia
    ...
    def normalize_team_name(name: str, round_id: int = None) -> str:
        ...
        # Pokud je round_id, zkus najít v DB
        if round_id:
            try:
                r = db.session.get(Round, round_id)
                if r:
                    # Najdi zápasy v tomto roundu
                    existing_matches = Match.query.filter_by(round_id=round_id).all()
                    all_teams = set()
            ...
            if action == "parse":
                ...
                for match_data in matches:
                    try:
                        ...
                        errors.append(f"{home_team} - {away_team}: {str(e)}")
                ...
                if errors:
                    flash(f"⚠️ Importováno {imported} zápasů, {len(errors)} chyb: {', '.join(errors[:3])}", "warning")
                else:
                    flash(f"✅ Úspěšně importováno {imported} zápasů!", "ok")
            
                audit("smart_import", "Match", None, count=imported, round=r.name)
