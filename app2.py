        ...
        <td style="text-align:center;">
          <div style="display:flex; justify-content:center; gap:6px; flex-wrap:nowrap;">
              ...
              {% if u.id != current_user.id and not u.is_owner %}
                ...
                <a class="btn btn-sm btn-danger" href="{{ url_for('admin_user_delete', user_id=u.id) }}"
                   onclick="return confirm('Opravdu smazat uživatele {{ u.username }}?')"
                   style="background:rgba(255,77,109,0.2); color:#ff4d6d;">Smazat</a>
              {% endif %}
          </div>
        </td>
      </tr>
