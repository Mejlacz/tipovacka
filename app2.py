...
""", u=u)
    ...
    def _parse_multiline_ocr_format(text: str) -> List[Dict]:
        ...
        classified = []  # ('dt', datetime) nebo ('team', str) nebo ('skip', ...)
        for raw in lines:
            raw = raw.strip()
            ...
            dt = parse_dt(raw)
            if dt:
                classified.append(('dt', dt))
            else:
                clean = clean_team(strip_prefix(raw))
                if clean and len(clean) >= 3:
                    classified.append(('team', clean))

        # Nyní hledáme vzory: team, dt, team  NEBO  dt, team, team
        results = []
        i = 0
        while i < len(classified):
            # Vzor 1: team1, dt, team2  (formát z Fortuny/iSport appky)
            if (i + 2 < len(classified)
                ...
                i += 3
                continue
            # Vzor 2: dt, team1, team2  (čistý screenshot)
            if (i + 2 < len(classified)
