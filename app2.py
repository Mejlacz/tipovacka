...
""", u=u)
    ...
    def normalize_team_name(name: str, round_id: int = None) -> str:
        ...
        if round_id:
            try:
                # Use with_entities to get only team names, not full Match objects
                existing_matches = db.session.query(Match.home_team, Match.away_team)\
                    .filter_by(round_id=round_id)\
                    .all()
                
                all_teams = set()
                for home, away in existing_matches:
                    if home:
                        all_teams.add(home)
                    if away:
                        all_teams.add(away)
