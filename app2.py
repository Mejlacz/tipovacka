...
def fetch_football_games(league_id: int, api_key: str, season: int = None) -> List[Dict]:
    ...
    Args:
        ...
        api_key: API klíč
        season: Rok sezóny (např. 2024 pro sezónu 2024/25)
    ...
    try:
        if season is None:
            # Pro fotbalové ligy, které běží přes léto (srpen-květen)
            # pokud je leden-červenec, použij předchozí rok
            now = datetime.now()
            if now.month < 8:  # Leden-Červenec
                season = now.year - 1
            else:  # Srpen-Prosinec
                season = now.year
