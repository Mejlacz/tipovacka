...
""", u=u)
    ...
    def _parse_single_line(line: str, line_num: int = 0) -> Optional[Dict]:
        """
        IMPROVED: Better handling of different formats + OCR compact dates
        
        Priority:
        1. Table format (no spaces): 27. 2. 2026DuklaSlavia18:00
        2. Compact date (OCR): 7.3.2026 Teplice Dukla 15:00
        3. Date with spaces: 27. 2. 2026 Dukla - Slavia 18:00
        4. Other formats
        """
        ...
        if len(parts) >= 4 and _is_date(parts[0]):
            return _parse_compact_date_format(line_normalized, parts)
        
        # TRY 4: Date at start with spaces: "27. 2. 2026 Dukla - Slavia 18:00"
        if len(parts) >= 3 and _is_date(' '.join(parts[:3])):
            return _parse_date_first_format(line_normalized, parts)
        
        # TRY 5: Date + time first: "27. 2. 20:00 Sparta vs Slavia"
        if len(parts) >= 4:
    ...
    def _parse_compact_date_format(line: str, parts: List[str]) -> Optional[Dict]:
        ...
        "27.3.2026 Sparta Slavia 18:30"
        
        Format: DATE TEAM1 TEAM2 TIME
        or: DATE TEAM1 TEAM2 (without time)
        """
        date_str = parts[0]  # "7.3.2026"
        rest_parts = parts[1:]  # ["Teplice", "Dukla", "15:00"]
        ...
        if rest_parts and re.match(r'^\d{1,2}:\d{2}$', rest_parts[-1]):
            ...
            team_parts = rest_parts[:-1]  # Everything except time
        else:
        ...
        # Split teams - could be:
        # ["Teplice", "Dukla"] - simple
        # ["Ml.", "Boleslav", "Jablonec"] - multi-word team
        
        # Simple case: exactly 2 parts
        if len(team_parts) == 2:
            home_team = team_parts[0]
            away_team = team_parts[1]
        # Multiple parts: try to find split point
        else:
            # Use smart team splitting
        ...
        # Update datetime with time if present
        if time_str and dt:
            try:
