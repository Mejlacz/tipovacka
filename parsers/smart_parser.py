...
def smart_parse_matches(text: str, round_id: int = None) -> List[Dict]:
    """
    🤖 ULTRA SMART PARSER V3 - Multi-format support
    
    Podporované formáty:
    1. Multi-line app format (with -- separators) - NOVÉ!
    2. OCR from screenshots
    3. Table format, CSV, Fortuna, UEFA, etc.
    ...
    # TRY 2: Clean OCR artifacts
    text = _clean_ocr_artifacts(text)
    
    # TRY 3: Handle table copy/paste
    text = _split_joined_lines(text)
    
    matches = []
    lines = text.strip().split('\n')
    
    print(f"🤖 Smart Parser V3: Zpracovávám {len(lines)} řádků")
    ...
    print(f"✅ Smart Parser V3: Nalezeno {len(matches)} zápasů")
    return matches
